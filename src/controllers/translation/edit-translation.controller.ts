...
import { TErrorResponse, TStructure, TTranslation, TTranslationModel, TValueTranslation } from '@/types/types';
...
export default async function UpdateTranslation(translationInput: TTranslationInput): Promise<{translation: TTranslation|null, userErrors: any}> {
    try {
        const {id, userId, projectId, structureId, subjectId, ...translationBody} = translationInput;
