...
export default async function CreateEntry(entryInput: TEntryInput): Promise<{entry: TEntry|null, userErrors: any}> {
    try {
        const {projectId, structureId, userId, doc: docBody} = entryInput;
        ...
        const {errors: errorsDB, data: savedData} = await (async (data) => {
            try {
                ...
                const entry: TEntryModel|null = await Entry.create({doc: data.entry, projectId, structureId, userId, createdBy: userId, updatedBy: userId});
